...
# def add_employment_to_pcd_sectors(lsoas, postcode_sectors):
#     """
#     Add the LAD indicator(s) to the relevant postcode sector.
...
#     for lsoa in tqdm(lsoas):
#         print(shape(lsoa['geometry']).bounds)
#         for n in idx.intersection((shape(lsoa['geometry']).bounds), objects=True):
#             print(n)
#             lsoa_centroid = shape(lsoa['geometry'])
#             # postcode_sector_shape = shape(lsoa['geometry'])
#             postcode_sector_shape = shape(n.object['geometry'])
#             if lsoa_centroid.intersects(postcode_sector_shape):
#                 print('here')
#                 # employement += lsoa['properties']['count']
...
if __name__ == "__main__":
    ...
    lsoa_shapes = os.path.join(BASE_PATH, 'shapes',
        'lsoas_ew_27700.shp')

    print('Loading local authority district shapes')
    lsoas = read_lsoa_shapes(lsoa_shapes)

    employment_path = os.path.join(BASE_PATH, 'employment', 'employment.csv')

    employment = load_employement_data(employment_path)
    ...
    # print('Loading postcode sector shapes')
    # path = os.path.join(DATA_RAW, 'shapes', 'PostalSector.shp')
    # postcode_sectors = read_postcode_sectors(path)#[:10]
